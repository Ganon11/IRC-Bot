      ...
      while len(channels):
         ...
         if receive:
            ...
            if -1 != buff.find('\n'):
               ...
               elif 'QUIT' == components['action'] and -1 != components['arguments'].find('ERROR :Closing Link') and -1 != components['arguments'].find('Ping timeout: '):
                  channels[:] = []
...
def main():
   valid_cfg = check_cfg(config.owner, config.server, config.nicks, config.real_name, config.log, config.cmds)
   ...
   try:
      logging.basicConfig(filename=os.path.join(config.log, logfile), level=config.logging_level, format='%(asctime)s - %(levelname)s - %(message)s', datefmt='%Y-%m-%d %H:%M:%S')
   except IOError as e:
