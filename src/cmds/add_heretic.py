...
def add_heretic(components):
	response = ''
	match = heretic_functions.singular_heretic_pattern.search(components['arguments'])
	if match:
		...
		match = heretic_functions.plural_heretic_pattern.search(components['arguments'])
		if match:
			target = match.groups()[0]
			if not components['sender'] == target:
				heretic_functions.build_heretics_db()
